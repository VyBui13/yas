...
* **Lệnh chạy**:
  ...
  # Bước 2: Thực hiện cuộc gọi từ product sang cart
  kubectl exec -it deployment/product -n yas -c product -- curl -v http://cart:80/actuator/health
  
  # Bước 3: Xem log của istio-proxy trong pod product để kiểm tra số lần thử lại
  kubectl logs deployment/product -c istio-proxy -n yas --tail=100 | grep -i "cart"
...
* **Lệnh chạy**:
  ```bash
  kubectl run test-deny --image=curlimages/curl -n yas -i --tty --rm -- curl -I http://cart:80/actuator/health
  ```
* **Kết quả thành công dự kiến**: Lớp bảo mật Istio chặn đứng request của pod dùng service account mặc định ngay tại proxy và trả về lỗi **`403 Forbidden`**:
  ```text
