...
* **Lệnh chạy**:
  ```bash
  kubectl run curl-test --image=curlimages/curl -n default -i --tty --rm -- curl -I http://cart.yas.svc.cluster.local:80/actuator/health
  ```
...
* **Lệnh chạy**:
  ```bash
  kubectl exec -it deployment/storefront-bff -n yas -c storefront-bff -- curl -I http://cart:80/actuator/health
  ```
...
* **Mục tiêu**: Kiểm tra xem khi service `cart` bị lỗi 500, proxy của pod gọi (`storefront-bff`) có tự động gửi lại request 3 lần trước khi trả lỗi về ứng dụng hay không.

#### A. Các bước thực hiện & Vị trí chạy lệnh:
* **Vị trí**: Terminal SSH của máy ảo Azure.
* **Lệnh chạy**:
  ```bash
  # Bước 1: Scale database PostgreSQL về 0 để làm cart mất kết nối DB (gây lỗi 500)
  kubectl scale statefulset postgresql -n postgres --replicas=0
  
  # Đợi 10 giây...
  
  # Bước 2: Thực hiện cuộc gọi từ storefront-bff sang cart
  kubectl exec -it deployment/storefront-bff -n yas -c storefront-bff -- curl -v http://cart:80/actuator/health
  
  # Bước 3: Xem log của istio-proxy trong pod storefront-bff để kiểm tra số lần thử lại
  kubectl logs deployment/storefront-bff -c istio-proxy -n yas --tail=100 | grep -i "cart"
...
Trong log xuất ra ở **Bước 3**, bạn sẽ thấy **3 dòng log** liên tiếp thể hiện Envoy proxy đang gửi lại request đến `cart` ở các mốc thời gian cách nhau 2 giây trước khi chính thức dừng cuộc gọi:
```text
[2026-06-23T...] "GET /actuator/health HTTP/1.1" 500 - "-" ... outbound|80||cart.yas.svc.cluster.local
[2026-06-23T...] "GET /actuator/health HTTP/1.1" 500 - "-" ... outbound|80||cart.yas.svc.cluster.local (retry 1)
[2026-06-23T...] "GET /actuator/health HTTP/1.1" 500 - "-" ... outbound|80||cart.yas.svc.cluster.local (retry 2)
[2026-06-23T...] "GET /actuator/health HTTP/1.1" 500 - "-" ... outbound|80||cart.yas.svc.cluster.local (retry 3)
```

---

### 3. Kiểm thử Authorization Policy (Zero-Trust Security)
* **Mục tiêu**: Kiểm tra chính sách phân quyền: Chỉ cho phép `storefront-bff` và `backoffice-bff` kết nối đến `cart`. Chặn tất cả các service khác.
...
* **Lệnh chạy**:
  ```bash
  kubectl exec -it deployment/product -n yas -c product -- curl -I http://cart:80/actuator/health
  ```
* **Kết quả thành công dự kiến**: Lớp bảo mật Istio chặn đứng request của pod `product` ngay tại proxy và trả về lỗi **`403 Forbidden`**:
  ```text
